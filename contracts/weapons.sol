...
contract Weapons is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function initialize () public initializer {
        __ERC721_init("Folk Warriors weapon", "FKW");
        __AccessControl_init_unchained();

        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);

        mintWeponCountReward=0;
        maxDurability = 20;
        secondsPerDurability = 3000;
    }
    ...
    */
    ...
    mapping(address => uint256) mintcount; //leovicio mint count
    uint public mintWeponCountReward; //leovicio
    ...
    uint256 public secondsPerDurability; //50 * 60
    ...
    //leovicio mint weapon
    function setmintWeponCountReward(uint256 number) external isAdmin {
        mintWeponCountReward = number;
    }
    ...
    function mint(address minter, uint256 seed) public restricted returns(uint256) {
        ...
        if(mintWeponCountReward != 0){//leovicio mint reward
        if(mintcount[minter] == 50 && mintWeponCountReward == 1) {
           stars = 3;
        }else if(mintcount[minter] == 100) {
           stars = 4;
        }
        }

        if(mintcount[minter] <= 99){  //leovicio mint count add and reset
        mintcount[minter] = mintcount[minter].add(1);
        }else{
            mintcount[minter] = 1;
        }
    ...
    function setBurnPointMultiplier(uint256 multiplier) external isAdmin {
        burnPointMultiplier = multiplier;
    }
    function setLowStarBurnPowerPerPoint(uint256 powerPerBurnPoint) external isAdmin {
        lowStarBurnPowerPerPoint = powerPerBurnPoint;
    }
    function setFourStarBurnPowerPerPoint(uint256 powerPerBurnPoint) external isAdmin {
        fourStarBurnPowerPerPoint = powerPerBurnPoint;
    }
    function setFiveStarBurnPowerPerPoint(uint256 powerPerBurnPoint) external isAdmin {
    ...
    function setDurabilityTimestamp(uint256 id, uint64 timestamp) external isAdmin {
        durabilityTimestamp[id] = timestamp;
    }

    //leovicio
    function setDurabilityVars(uint256 max, uint256 secondsPD) external isAdmin {
        maxDurability = max;
        secondsPerDurability = secondsPD;
    ...
    function getDurabilityMaxWait() public view returns (uint64) {
