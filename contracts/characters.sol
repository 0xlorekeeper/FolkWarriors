...
contract Characters is Initializable, ERC721Upgradeable, AccessControlUpgradeable, ITransferCooldownable {
    ...
    function initialize () public initializer {
        __ERC721_init("Folk Warriors character", "FKC");
        __AccessControl_init_unchained();

        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(GAME_ADMIN, msg.sender);
        maxStamina = 200;
        secondsPerStamina = 150;
    }
    ...
    */
    ...
    //leovicio
    uint256 public maxStamina;
    uint256 public secondsPerStamina; //5 * 60
    ...
    //leovicio
    function setStaminadata(uint256 max,uint256 sperstamina) external isAdmin {
        maxStamina = max;
        secondsPerStamina = sperstamina;
    }
}
