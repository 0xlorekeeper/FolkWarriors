...
contract CryptoBlades is Initializable, AccessControlUpgradeable {
    ...
    int128 public constant REWARDS_CLAIM_TAX_MAX = 5534023222118654840; // = ~0.15 = ~15%  se buga foi isso
    uint256 public constant REWARDS_CLAIM_TAX_DURATION = 30 days;
    ...
    //leovicio fix taxa
    uint256 public REWARDS_CLAIM_TAX_DURATIONFIX;
    ...
    function migrateTo_reward_claim5d() external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");
        REWARDS_CLAIM_TAX_DURATIONFIX = 5 days;
    }
    ...
    //leovicio reward
    function setRewardClainDurationFx(uint256 dayf) public restricted {
        REWARDS_CLAIM_TAX_DURATIONFIX = dayf;
    }
    ...
    function _getRewardsClaimTax(address playerAddress) internal view returns (int128) {
        ...
        uint256 rewardsClaimTaxTimerEnd = _rewardsClaimTaxTimerStart[playerAddress].add(REWARDS_CLAIM_TAX_DURATIONFIX);

        (, uint256 durationUntilNoTax) = rewardsClaimTaxTimerEnd.trySub(block.timestamp);

        assert(0 <= durationUntilNoTax && durationUntilNoTax <= REWARDS_CLAIM_TAX_DURATIONFIX);

        int128 frac = ABDKMath64x64.divu(durationUntilNoTax, REWARDS_CLAIM_TAX_DURATIONFIX);
